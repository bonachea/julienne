...
program pure_stop_and_print
...
#ifndef __GFORTRAN__
  if (      command_line%string_argument_present( [string_t("--help"), string_t("-h")                                 ] ))       stop usage_info()
  if (.not. command_line%string_argument_present( [string_t("--file"), string_t("--array"), string_t("--derived-type")] )) error stop usage_info()
#else
  block
  ...
  associate(file_name => command_line%character_flag_value("--file"))
    if (len(file_name) > 0) call pure_subroutine(.false., .false., file_t(file_name))
  end associate
  call pure_subroutine(command_line%character_argument_present(["--array"]), command_line%character_argument_present(["--derived-type"]))
